  ...
  return (
    <main className="screen-enter flex-1 flex flex-col min-w-0">
      ...
      <div className="flex items-center justify-between shrink-0" style={{ padding: '20px 28px', borderBottom: `1px solid ${t.borderSoft}` }}>
        <div>
          <h1 className="m-0" style={{ fontSize: 22, fontWeight: 600, color: t.text, letterSpacing: -0.4 }}>Analytics</h1>
      ...
      <div className="flex-1 overflow-y-auto flex flex-col" style={{ padding: 28, gap: 22 }}>
        ...
        <div className="grid gap-[18px]" style={{ gridTemplateColumns: '1.4fr 1fr' }}>
          ...
          >
            ...
            <div className="flex mt-2" style={{ fontSize: 10, color: t.textFaint, fontFamily: '"JetBrains Mono", monospace' }}>
              {['12a', '6a', '12p', '6p'].map((l, i) => (
                <div key={l} className="flex-1" style={{ textAlign: i === 0 ? 'left' : 'center' }}>{l}</div>
              ))}
            </div>
          </div>
          ...
          >
            ...
            {a.timeByTag.length > 0 ? a.timeByTag.map((row, i) => (
              <div key={row.tag} style={{ marginBottom: i === a.timeByTag.length - 1 ? 0 : 12 }}>
                ...
                <div className="h-1.5 rounded-[3px] overflow-hidden" style={{ background: t.borderSoft }}>
                  <div style={{
