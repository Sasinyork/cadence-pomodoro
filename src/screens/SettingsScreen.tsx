...
function Section({ t, title, hint, children }: { t: SurfaceTokens; title: string; hint?: string; children: ReactNode }) {
  return (
    <div style={{
    ...
    <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
      ...
      <div style={{ flex: 1, padding: 28, overflowY: 'auto', display: 'grid', gridTemplateColumns: 'minmax(0, 640px)', justifyContent: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, width: '100%' }}>
          <Section t={t} title="Timer durations" hint="The intervals that shape one full cycle.">
          ...
          <Section t={t} title="Automation" hint="Let sessions flow without manual restarts.">
          ...
          <Section t={t} title="Sound & notifications">
            ...
            <Row t={t} label="Ambient track" hint="Background sound during focus blocks"
              control={
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: t.surfaceAlt, border: `1px solid ${t.border}`, borderRadius: 8, padding: '6px 12px', height: 38, minWidth: 160 }}>
                  <span style={{ fontSize: 13, color: t.text, fontWeight: 500, flex: 1 }}>{s.ambientTrack}</span>
                  <Icons.chev size={12} color={t.textMuted} />
                </div>
              }
            />
          </Section>

          <Section t={t} title="Timer visualization">
